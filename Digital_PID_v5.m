function varargout = Digital_PID_v5(varargin)
% DIGITAL_PID_V5 MATLAB code for Digital_PID_v5.fig
%      DIGITAL_PID_V5, by itself, creates a new DIGITAL_PID_V5 or raises the existing
%      singleton*.
%
%      H = DIGITAL_PID_V5 returns the handle to a new DIGITAL_PID_V5 or the handle to
%      the existing singleton*.
%
%      DIGITAL_PID_V5('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DIGITAL_PID_V5.M with the given input arguments.
%
%      DIGITAL_PID_V5('Property','Value',...) creates a new DIGITAL_PID_V5 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Digital_PID_v5_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Digital_PID_v5_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Digital_PID_v5_OpeningFcn, ...
                   'gui_OutputFcn',  @Digital_PID_v5_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Digital_PID_v5 is made visible.
function Digital_PID_v5_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Digital_PID_v5 (see VARARGIN)

% Choose default command line output for Digital_PID_v5
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Digital_PID_v5 wait for user response (see UIRESUME)
...
function varargout = Digital_PID_v5_OutputFcn(hObject, eventdata, handles) 
...
[t PlantOutput ref_size]=sim_response(sim_time, t_ref, t_dist, c_r, c_D, Ts, 'Plant output', a1);
reference=ones(ref_size);
reference=padarray(reference, t_ref, 0, 'pre');
%stairs(handles.axes1, t,PlantOutput);
plot(a1, t,reference,'.', t,PlantOutput,'o');
a1.XLabel.String = 'Time (sec)'; a1.YLabel.String = 'Amplitude';
set(a1, 'XMinorGrid','on', 'YMinorGrid','on');
title(handles.axes1, sprintf('Plant output, step disturbance at %g s', t_dist*Ts));

[t ControlSignal ref_size]=sim_response(sim_time, t_ref, t_dist, U_r, U_D, Ts, 'Control signal', a2);
%stairs(handles.axes2, t,ControlSignal);
plot(a2, t,ControlSignal,'o');
a2.XLabel.String = 'Time (sec)'; a2.YLabel.String = 'Amplitude';
